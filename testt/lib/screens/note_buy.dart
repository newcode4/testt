...
class NoteBuyState extends State<NoteBuy> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget myAppBar() {
      return AppBar(
        title: Text('매매일지', style: Theme.of(context).textTheme.headline5),
        centerTitle: true,
        elevation: 0,
            ...
            : IconButton(
                ...
                onPressed: () async {
                  Note result = await showSearch(
                      context: context,
                      delegate: NotesSearch(notes: noteList2));
                  if (result != null) {
                      ...
                      Expanded(
                        child: Text(
                          this.noteList[index].total == null //수정 다큐멘터리에서
                              ? ''
                              : '매수가 : ${f.format(int.parse(this.noteList[index].price))}원\n'
                                  '평가금액 : ${f.format(int.parse(this.noteList[index].total))}원',
    ...
    return StaggeredGridView.countBuilder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return GestureDetector(
          onLongPress: () {
            _delete(context, this.noteList2[index]);
          },
          child: Padding(
            padding: const EdgeInsets.all(8.0),
            child: Container(
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        Expanded(
                          child: Text(
                            '거래차익 : ${f.format(int.parse(this.noteList2[index].total))}원',
                            style: Theme.of(context).textTheme.bodyText1,
                          ),
                        ),
                      ],
...
FinishDiaLog(context, String title, String total, String date) {
  ...
  dbFuture.then((database) {
    Future<List<Note>> noteListFuture = databaseHelper.getNoteList2();
    noteListFuture.then((noteList) {
      // print(noteList[0].total);
      var f = NumberFormat('###,###,###,###');
      var test = [];

      //   DatabaseBuy().getNoteList2().then((value) =>
